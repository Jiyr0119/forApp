var url = 'https://test20.1haomei.com/shop/',
	token;
var request = new HproseHttpClient(url+'Distribution.php', ['getnotProfitOrder'],{ timeout: 20000 });
            ...
            let res = maplistToArr(result),
            	...
            	res.map(function(item) {
					  ...
					  content +=
					    ...
					    item.order_sn +
					    '</p><p style="color:#ff4242;margin-right: .2rem;">' +
					    item.status_msg +
					    "</p></div>";
					  item.order_goods.map(function(item) {
					    content +=
					      ...
					      item.goods_name +
					      '</div><div style="color:#ff4242;margin-top: .2rem;box-sizing: border-box;"><text style="font-size:.36rem;padding-top: .25rem;display: inline-block;">\xA5' +
					      item.goods_price +
					  ...
					  content +=
					    ...
					    item.pay_time +
					    '</div><div style="width: 2.5rem;"><div>\u5B9E\u4ED8\u6B3E\uFF1A<text style="color:#ff4242;">\xA5' +
					    item.order_amount +
